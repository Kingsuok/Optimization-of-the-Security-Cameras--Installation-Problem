...
int input(int inPut[],int n){
	...
	while (1){
		...
		if (inPut1[0]=='V'){
			if (inPut1[2]=='-'){
				printf("%s\n","Error: the number shoule not be negative.");fflush(stdout);
				continue;
			}
			else{
				...
				if (flag1==0){
					...
					printf("%s\n","Error: V has been inputed.");fflush(stdout);
					continue;
				}
			}
		}
		if (inPut1[0]=='E'){
			...
			if (flag1==1){
				if(negative(inPut1,sizeof(inPut1))){
					flag1=0;
					printf("%s\n","Error:the number is not negative.");fflush(stdout);
					continue;
				}
				printf("E = %s\n",p2+2);fflush(stdout);
				if(inPut1[3]=='}'){
				...
				for (p1=inPut1;*p1!='}';p1++){
					if (*p1=='<'){
						...
						if (buf1<=(vertexes-1)&&buf2<=(vertexes-1)){
							...
							i+=2;
							if(buf1==buf2){
                            printf("%s\n","Error: E is not correct.");fflush(stdout);
							flag1=0;
							break;
							}
						}else
			...
			{
				printf("%s\n","Error:V should be inputed firstly.");fflush(stdout);
				continue;
			}
		}   
		if (flagSquence==0){
			printf("%s\n","Error:V,E should be inputted at first.");fflush(stdout);
			continue;
		}
		if(inPut1[0]=='s'){
                        if(vertexes==0){
                        printf("%s\n","Error:there are not any vertexes.");
                        fflush(stdout);
			continue;
			}
			if(negative(inPut1,sizeof(inPut1))){
			...
			if(inOrOut( buf1,buf2,inPut,2*edges)){
				...
				printf("%s\n","Error:s is not correct.");fflush(stdout);
				continue;
			}
		}
	}
